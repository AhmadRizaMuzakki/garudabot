...
/**
 * Encode a RaceroBlockComment as XML.
 * @param {!Blockly.RaceroBlockComment} block The block possibly containing
 *     a comment to encode.
...
Blockly.Xml.scratchCommentToDom_ = function(block, element) {
  ...
  if (commentText) {
    ...
    if (typeof block.comment == 'object') {
      ...
      var hw;
      if (block.comment instanceof Blockly.RaceroBlockComment) {
  ...
  for (var i = 0, xmlChild; xmlChild = xmlBlock.childNodes[i]; i++) {
    ...
    switch (xmlChild.nodeName.toLowerCase()) {
      ...
      case 'comment':
        ...
        // Note bubbleX and bubbleY can be NaN, but the RaceroBlockComment
