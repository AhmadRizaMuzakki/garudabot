...
Blockly.BlockSvg.prototype.render = function(opt_bubble) {
  ...
  for (var i = 0; i < icons.length; i++) {
    if (icons[i] instanceof Blockly.RaceroBlockComment) {
  ...
  if (this.isRaceroExtension && this.outputConnection) {
    cursorX += this.RTL ?
  ...
  // Position the Racero Block Comment Icon at the end of the block
...
Blockly.BlockSvg.prototype.renderFields_ = function(fieldList, cursorX,
  ...
  for (var t = 0, field; field = fieldList[t]; t++) {
    ...
    if (this.isRaceroExtension && (field === this.inputList[0].fieldRow[0])
...
Blockly.BlockSvg.prototype.renderCompute_ = function(iconWidth) {
  ...
  for (var i = 0, input; input = inputList[i]; i++) {
    ...
    // See github.com/LLK/racero-blocks/issues/1658
...
Blockly.BlockSvg.prototype.computeInputHeight_ = function(input, row,
  ...
  if (this.inputList.length === 1 && this.outputConnection &&
    ...
    // make it taller.
    if (this.isRaceroExtension && this.previousConnection) {
...
Blockly.BlockSvg.prototype.computeOutputPadding_ = function(inputRows) {
  ...
  if (firstField) {
    ...
    // https://github.com/LLK/racero-blocks/issues/376
...
Blockly.BlockSvg.prototype.renderDrawLeft_ = function(steps) {
  if (this.outputConnection) {
    // Racero-style reporters have output connection y at half block height.
