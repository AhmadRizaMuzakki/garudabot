...
goog.provide('Blockly.RaceroBlockComment');

goog.require('Blockly.Comment');
goog.require('Blockly.Events.BlockChange');
goog.require('Blockly.Events.Ui');
goog.require('Blockly.Icon');
goog.require('Blockly.RaceroBubble');
 ...
 * @constructor
 */
Blockly.RaceroBlockComment = function(block, text, id, x, y, minimized) {
  Blockly.RaceroBlockComment.superClass_.constructor.call(this, block);
  /**
  ...
  if (!block.rendered) {
    Blockly.RaceroBlockComment.fireCreateEvent(this);
  }
  // If the block is rendered, fire event the create event when the comment is made
  // visible
};
goog.inherits(Blockly.RaceroBlockComment, Blockly.Comment);

/**
 * Width of bubble.
 * @private
 */
Blockly.RaceroBlockComment.prototype.width_ = 200;

/**
 * Height of bubble.
 * @private
 */
Blockly.RaceroBlockComment.prototype.height_ = 200;

/**
 * Comment Icon Size.
 * @package
 */
Blockly.RaceroBlockComment.prototype.SIZE = 0;

/**
 * Offset for text area in comment bubble.
 * @private
 */
Blockly.RaceroBlockComment.TEXTAREA_OFFSET = 12;
 ...
 * @private
 */
Blockly.RaceroBlockComment.MAX_LABEL_LENGTH = 12;

/**
 * Maximum character length for comment text.
 * @private
 */
Blockly.RaceroBlockComment.COMMENT_TEXT_LIMIT = 8000;

/**
 * Width that a minimized comment should have.
 * @private
 */
Blockly.RaceroBlockComment.MINIMIZE_WIDTH = 200;
 ...
 * @private
 */
Blockly.RaceroBlockComment.prototype.drawIcon_ = function(_group) {
  // NO-OP -- Don't render a comment icon for Racero block comments
};
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.renderIcon = function(cursorX, topMargin) {
 ...
 * @private
 */
Blockly.RaceroBlockComment.prototype.createEditor_ = function() {
  this.foreignObject_ = Blockly.utils.createSvgElement('foreignObject',
      {
        'x': Blockly.RaceroBubble.BORDER_WIDTH,
        'y': Blockly.RaceroBubble.BORDER_WIDTH + Blockly.RaceroBubble.TOP_BAR_HEIGHT,
  ...
  textarea.setAttribute('maxlength', Blockly.RaceroBlockComment.COMMENT_TEXT_LIMIT);
  textarea.setAttribute('placeholder', Blockly.Msg.WORKSPACE_COMMENT_DEFAULT_TEXT);
  body.appendChild(textarea);
  this.textarea_ = textarea;
  this.textarea_.style.margin = (Blockly.RaceroBlockComment.TEXTAREA_OFFSET) + 'px';
 ...
 * @private
 */
Blockly.RaceroBlockComment.prototype.textareaFocus_ = function(e) {
  Blockly.RaceroBlockComment.superClass_.textareaFocus_.call(this, e);
 ...
 * @private
 */
Blockly.RaceroBlockComment.prototype.resizeBubble_ = function() {
  if (this.isVisible() && !this.isMinimized_) {
    var size = this.bubble_.getBubbleSize();
    var doubleBorderWidth = 2 * Blockly.RaceroBubble.BORDER_WIDTH;
    var textOffset = Blockly.RaceroBlockComment.TEXTAREA_OFFSET * 2;
    this.foreignObject_.setAttribute('width', size.width - doubleBorderWidth);
    this.foreignObject_.setAttribute('height', size.height - doubleBorderWidth - Blockly.RaceroBubble.TOP_BAR_HEIGHT);
    this.textarea_.style.width = (size.width - textOffset) + 'px';
    this.textarea_.style.height = (size.height - doubleBorderWidth -
       Blockly.RaceroBubble.TOP_BAR_HEIGHT - textOffset) + 'px';
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.updateColour = function() {
  if (this.isVisible()) {
 ...
 * @private
 */
Blockly.RaceroBlockComment.prototype.autoPosition_ = function() {
  ...
  if (this.isMinimized_) {
    ...
    this.y_ = this.iconXY_.y - (Blockly.RaceroBubble.TOP_BAR_HEIGHT / 2);
  } else {
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.setVisible = function(visible) {
  ...
  if (visible) {
    ...
    // Create the bubble.
    this.bubble_ = new Blockly.RaceroBubble(
 ...
 * @private
 */
Blockly.RaceroBlockComment.prototype.toggleMinimize_ = function() {
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.setMinimized = function(minimize) {
  ...
  if (minimize) {
    ...
    this.setBubbleSize(Blockly.RaceroBlockComment.MINIMIZE_WIDTH,
        Blockly.RaceroBubble.TOP_BAR_HEIGHT);
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.setBubbleSize = function(width, height) {
  if (this.bubble_) {
    if (this.isMinimized_) {
      this.bubble_.setBubbleSize(Blockly.RaceroBlockComment.MINIMIZE_WIDTH,
          Blockly.RaceroBubble.TOP_BAR_HEIGHT);
    } else {
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.setSize = function(width, height) {
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.getLabelText = function() {
  if (this.text_.length > Blockly.RaceroBlockComment.MAX_LABEL_LENGTH) {
    if (this.block_.RTL) {
      return '\u2026' + this.text_.slice(0, Blockly.RaceroBlockComment.MAX_LABEL_LENGTH);
    }
    return this.text_.slice(0, Blockly.RaceroBlockComment.MAX_LABEL_LENGTH) + '\u2026';
  } else {
    return this.text_;
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.setText = function(text) {
  if (this.text_ != text) {
    Blockly.Events.fire(new Blockly.Events.CommentChange(
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.moveTo = function(x, y) {
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.getXY = function() {
  if (this.bubble_) {
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.getHeightWidth = function() {
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.getBoundingRectangle = function() {
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.isMinimized = function() {
 ...
 * @private
 */
Blockly.RaceroBlockComment.prototype.showContextMenu_ = function(e) {
  var menuOptions = [];
 ...
 * @package
 */
Blockly.RaceroBlockComment.prototype.toXmlWithXY = function() {
 ...
 * @package
 */
Blockly.RaceroBlockComment.fireCreateEvent = function(comment) {
 ...
 */
Blockly.RaceroBlockComment.prototype.dispose = function() {
  if (Blockly.Events.isEnabled()) {
 ...
 */
Blockly.RaceroBlockComment.prototype.focus = function() {
