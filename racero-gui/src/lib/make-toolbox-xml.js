import RaceroBlocks from 'racero-blocks';
...
const motion = function (isInitialSetup, isStage, targetId, colors) {
    const stageSelected = RaceroBlocks.RaceroMsgs.translate(
...
const looks = function (isInitialSetup, isStage, targetId, costumeName, backdropName, colors) {
    const hello = RaceroBlocks.RaceroMsgs.translate('LOOKS_HELLO', 'Hello!');
    const hmm = RaceroBlocks.RaceroMsgs.translate('LOOKS_HMM', 'Hmm...');
    // Note: the category's secondaryColour matches up with the blocks' tertiary color, both used for border color.
    return `
...
const sensing = function (isInitialSetup, isStage, targetId, colors) {
    const name = RaceroBlocks.RaceroMsgs.translate('SENSING_ASK_TEXT', 'What\'s your name?');
    // Note: the category's secondaryColour matches up with the blocks' tertiary color, both used for border color.
    return `
    <category
...
const operators = function (isInitialSetup, isStage, targetId, colors) {
    const apple = RaceroBlocks.RaceroMsgs.translate('OPERATORS_JOIN_APPLE', 'apple');
    const banana = RaceroBlocks.RaceroMsgs.translate('OPERATORS_JOIN_BANANA', 'banana');
    const letter = RaceroBlocks.RaceroMsgs.translate('OPERATORS_LETTEROF_APPLE', 'a');
    // Note: the category's secondaryColour matches up with the blocks' tertiary color, both used for border color.
    return `
    <category
 ...
 * blocks with localized default parameters (e.g. ask and wait) should not be loaded. (LLK/racero-gui#5445)
 * @param {?boolean} isStage - Whether the toolbox is for a stage-type target. This is always set to true
 * when isInitialSetup is true.
 * @param {?string} targetId - The current editing target
 * @param {?Array.<object>} categoriesXML - optional array of `{id,xml}` for categories. This can include both core
 * and other extensions: core extensions will be placed in the normal Racero order; others will go at the bottom.
 ...
 * @returns {string} - a RaceroBlocks-style XML document for the contents of the toolbox.
 */
