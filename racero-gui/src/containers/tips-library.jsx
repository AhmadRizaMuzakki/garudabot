...
class TipsLibrary extends React.PureComponent {
    ...
    render () {
        const decksLibraryThumbnailData = Object.keys(decksLibraryContent)
            .filter(id => {
                if (notRaceroDesktop()) return true; // Do not filter anything in online editor
                const deck = decksLibraryContent[id];
                // Racero Desktop doesn't want tutorials with `requiredProjectId`
                if (Object.prototype.hasOwnProperty.call(deck, 'requiredProjectId')) return false;
                // Racero Desktop should not load tutorials that are _only_ videos
...
const mapStateToProps = state => ({
    visible: state.raceroGui.modals.tipsLibrary,
