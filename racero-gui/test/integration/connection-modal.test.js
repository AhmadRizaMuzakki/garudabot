...
// The tests below require Racero Link to be unavailable, so we can trigger
...
describe('Hardware extension connection modal', () => {
    ...
    test('Message saying Racero Link is unavailable (BLE)', async () => {
        await driver.quit();
        driver = getDriver();
        ...
        findByText('Racero Link'); // Racero Link is mentioned in the error modal

        const logs = await getLogs();
        await expect(logs).toEqual([]);
    });

    test('Message saying Racero Link is unavailable (BT)', async () => {
        await loadUri(uri);
