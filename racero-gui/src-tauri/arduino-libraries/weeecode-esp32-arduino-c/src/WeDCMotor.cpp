...
void WeDCMotor::reset(uint8_t port)
{
  if(port > 2)
  {
    ...
    pinMode(WetwoPort[port].s1, OUTPUT);
...
void WeDCMotor::run(int16_t speed)
{
 //   Serial.println(BOARD_TYPE);
  ...
  // Mini/Pro WeeeCode: speed 0 di cabang maju menulis analogWrite(255-speed)=255
  // (PWM terbalik) sehingga motor malah gas penuh. Hentikan dengan PWM 0.
  if(speed == 0)
  {
    if(onBoard_flag==1)
    {
  ...
  if(speed >= 0)
  {
   ...
   #ifdef ESP32
    else if(dc_dir_pin==22)  
	 {
      if(_boardType == 1) {  // Mini板
        digitalWrite(dc_dir_pin,HIGH);
        analogWrite(dc_pwm_pin, 255-speed);
      } else if(_boardType == 2) {  // Pro板
        digitalWrite(dc_dir_pin,LOW);
        analogWrite(dc_pwm_pin, speed);
      }
	  }
    #endif
   else if(dc_dir_pin == 19)
   {
    digitalWrite(dc_dir_pin,HIGH);
    analogWrite(dc_pwm_pin, 255-speed);
   }
   else if(dc_dir_pin==23)
   {
       digitalWrite(dc_dir_pin,HIGH);
       analogWrite(dc_pwm_pin, speed);    
   }
