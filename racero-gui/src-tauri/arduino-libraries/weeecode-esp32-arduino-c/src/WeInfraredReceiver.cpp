...
WeInfraredReceiver::WeInfraredReceiver(uint8_t IR_PIN){
    ir_pin=IR_PIN;
    ir_result=0;
    ir_resp = 0xFF;
    t0=0;

    ir_buttons[ 0].val = 0x45; ir_buttons[ 0].ch = 'A';
    ir_buttons[ 1].val = 0x46; ir_buttons[ 1].ch = 'B';
    ir_buttons[ 2].val = 0x47; ir_buttons[ 2].ch = 'C';
    ir_buttons[ 3].val = 0x44; ir_buttons[ 3].ch = 'D';
    ir_buttons[ 4].val = 0x40; ir_buttons[ 4].ch = 'U';
...
WeInfraredReceiver::~WeInfraredReceiver(){

}

void WeInfraredReceiver::begin(){
    if(ir_pin >= 0){
        pinMode(ir_pin, INPUT_PULLUP);
        attachInterrupt(ir_pin, isr_WeIR, CHANGE); // Interrupts will be handle by isr_IR
    }
}
void WeInfraredReceiver::defineButtons(irBtn_t* b){
...
void IRAM_ATTR WeInfraredReceiver::setIRresult(uint8_t result){
    ir_resp = result;
    IR_VALUE = result;
}
...
void WeInfraredReceiver::loop(){ // transform raw data from IR to ir_result
...
void IRAM_ATTR isr_WeIR()
{
    ...
    else if((intval > 2000) && (intval < 3500)){
        ir.ir_repeat = 1;
        return;
    }
