...
void WeWifi::mqtt_publish(const char * topic, char * message, uint8_t qos)
{
  int _send_len = 0;
...
// Blynk IOT
void WeWifi::blynk_http_update(const char *autho_token, char *multiple_data)
{
  String data_pkg;
  data_pkg = String("GET /external/api/batch/update?token=") + String(autho_token) + String("&") + String(multiple_data) + String(" HTTP/1.1\r\n\r\n");
  write(data_pkg, 0);
}

void WeWifi::blynk_http_get(const char *autho_token, char *multiple_data)
{
  String data_pkg;
  data_pkg = String("GET /external/api/get?token=") + String(autho_token) + String("&") + String(multiple_data) + String(" HTTP/1.1\r\n\r\n");
  write(data_pkg, 0);
}

String WeWifi::blynk_data_process(const char *virtual_pin)
{
  rx_buffer[rx_len] = 0x00;
  String data_pkg = String((char*)(strstr(rx_buffer, "\r\n\r\n") + 4));
  if (data_pkg.indexOf("{")>=0)
  {
    uint8_t start,end;
    start = data_pkg.indexOf(":", data_pkg.indexOf(virtual_pin)+1);
    if(data_pkg.indexOf(",", data_pkg.indexOf(virtual_pin)+1)>0){
        end = data_pkg.indexOf(",", data_pkg.indexOf(virtual_pin)+1);
    }else{
        end = data_pkg.indexOf("}", data_pkg.indexOf(virtual_pin)+1);
    }
    if (data_pkg.substring(start+1,end).indexOf("\"")>=0)
    {
      return data_pkg.substring(start+2,end-1);
    }
    return data_pkg.substring(start+1,end);
  }
  return data_pkg;
}
