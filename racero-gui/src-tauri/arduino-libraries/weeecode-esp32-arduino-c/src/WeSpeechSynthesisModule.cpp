...
HardwareSerial speech_serial(1);
WeSpeechSynthesisModule::WeSpeechSynthesisModule(uint8_t port)
{
  reset(port);
}

void WeSpeechSynthesisModule::reset(uint8_t port)
{
    speech_serial.begin(9600, SERIAL_8N1, -1, port);
}

void WeSpeechSynthesisModule::beginSpeek(uint8_t len)
{
  speech_serial.write(0xFD);   // 帧头
  speech_serial.write(((len+2)>>8) & 0xff);   // 数据长度
  speech_serial.write((len+2) & 0xff);        //

  speech_serial.write(0x01);    // 命令字
  speech_serial.write(0x01);    // 命令参数
}
...
//   beginSpeek(len);
//   for(int i=0; i<len; i++){   // 文本标记+文字字符串GBK
//     speech_serial.write(pgm_read_byte(&data_in[i]));
//   }
// }
