...
public:
  ...
  void mqtt_publish(const char * topic, char * message, uint8_t qos);
  void mqtt_ping(void);
  ...
  void blynk_http_update(const char *autho_token, char *multiple_data);
  void blynk_http_get(const char *autho_token, char *multiple_data);
  String blynk_data_process(const char *virtual_pin);
