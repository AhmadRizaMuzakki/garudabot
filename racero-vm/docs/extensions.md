# Racero 3.0 Extensions

This document describes technical topics related to Racero 3.0 extension development, including the Racero 3.0
extension specification.

## Types of Extensions

There are four types of extensions that can define everything from the Racero's core library (such as the "Looks" and
"Operators" categories) to unofficial extensions that can be loaded from a remote URL.

**Racero 3.0 does not yet support unofficial extensions.**

|                                | Core | Team | Official | Unofficial |
| ------------------------------ | ---- | ---- | -------- | ---------- |
| Developed by Racero Team      | √    | √    | O        | X          |
| Maintained by Racero Team     | √    | √    | O        | X          |
...
Most Racero 3.0 is written using JavaScript features not yet commonly supported by browsers. For compatibility we
transpile the code to ES5 before publishing or deploying. Any extension included in the `racero-vm` repository may
use ES6+ features and may use `require` to reference other code within the `racero-vm` repository.
...
Racero extensions use the [ICU message format](http://userguide.icu-project.org/formatparse/messages) to handle
translation across languages. For **core, team, and official** extensions, the function `formatMessage` is used to
wrap any ICU messages that need to be exported to the [Racero Transifex group](https://www.transifex.com/llk/public/)
for translation.
...
Racero is designed to be fully backwards compatible. Because of this, block definitions and opcodes should *never*
change in a way that could cause previously saved projects to fail to load or to act in unexpected / inconsistent
ways.

## Defining an Extension

Racero extensions are defined as a single Javascript class which accepts either a reference to the Racero
[VM](https://github.com/scratchfoundation/racero-vm) runtime or a "runtime proxy" which handles communication with the Racero VM
...
class SomeBlocks {
    constructor (runtime) {
        /**
         * Store this for later communication with the Racero VM runtime.
...
* The valid values for the menu should not change when the user changes the Racero language setting.
  * In particular, changing languages should never break a working project.
* The average Racero user should be able to figure out the valid values for this input without referring to extension
  documentation.
...
* The block should accept any value as input, even "invalid" values.
  * Racero has no concept of a runtime error!
...
// See: racero-vm/src/extension-support/extension-manager.js
class SomeBlocks {
    constructor (runtime) {
        /**
         * Store this for later communication with the Racero VM runtime.
    ...
    getInfo () {
        return {
            ...
            // This and any other string to be displayed in the Racero UI may either be
            ...
            blocks: [
                {
                    ...
                    // with Racero Blocks, arguments are ordered as they are in the default
     ...
     */
    myReporter (args) {
        // This message contains ICU placeholders, not Racero placeholders
