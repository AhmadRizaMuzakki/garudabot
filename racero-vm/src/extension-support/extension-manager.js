...
/**
 * @typedef {object} ConvertedBlockInfo - Raw extension block data paired with processed data ready for racero-blocks
 * @property {ExtensionBlockMetadata} info - the raw block info
 * @property {object} json - the racero-blocks JSON definition for this block
 * @property {string} xml - the racero-blocks XML definition for this block
 */
...
class ExtensionManager {
    ...
    /**
     * Retrieve the raw instance of a loaded extension.
     * @param {string} extensionId - the ID of the extension (e.g., 'pins')
     * @returns {object|null} - the extension instance, or null if not found
     */
    getExtensionInstance (extensionId) {
        // 1. Check if the extension is actually loaded
        if (!this._loadedExtensions.has(extensionId)) {
            return null;
        }

        // 2. Get the internal service name (e.g., 'extension_0_pins')
        const serviceName = this._loadedExtensions.get(extensionId);

        // 3. Extract the raw javascript class instance directly from the dispatcher
        return dispatch.services[serviceName];
    }
    ...
    _prepareMenuInfo (serviceName, menus) {
        ...
        for (let i = 0; i < menuNames.length; i++) {
            ...
            if (typeof menuInfo.items === 'string') {
                ...
                // Bind the function here so we can pass a simple item generation function to Racero Blocks later.
     ...
     * @returns {Array} menu items ready for racero-blocks.
     * @private
     */
