...
/**
 * Host for the Pen-related blocks in Racero 3.0
 * @param {Runtime} runtime - the runtime instantiating this block package.
 * @constructor
 */
class Racero3PenBlocks {
    constructor (runtime) {
        /**
    ...
    _clampPenSize (requestedSize) {
        return MathUtil.clamp(
            requestedSize,
            Racero3PenBlocks.PEN_SIZE_RANGE.min,
            Racero3PenBlocks.PEN_SIZE_RANGE.max
        );
    }
     ...
     */
    _getPenState (target) {
        let penState = target.getCustomState(Racero3PenBlocks.STATE_KEY);
        if (!penState) {
            penState = Clone.simple(Racero3PenBlocks.DEFAULT_PEN_STATE);
            target.setCustomState(Racero3PenBlocks.STATE_KEY, penState);
        }
        return penState;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const penState = sourceTarget.getCustomState(Racero3PenBlocks.STATE_KEY);
            if (penState) {
                newTarget.setCustomState(Racero3PenBlocks.STATE_KEY, Clone.simple(penState));
                if (penState.penDown) {
    ...
    /* LEGACY OPCODES */
    /**
     * Racero 2 "hue" param is equivelant to twice the new "color" param.
     * @param {object} args - the block arguments.
    ...
    /**
     * Update the pen state's color from its hue & shade values, Racero 2.0 style.
...
module.exports = Racero3PenBlocks;
