...
/**
 * Class for the motion-related blocks in Racero 3.0
 * @param {Runtime} runtime - the runtime instantiating this block package.
 * @constructor
 */
class Racero3VideoSensingBlocks {
    constructor (runtime) {
        /**
    ...
    reset () {
        ...
        for (let i = 0; i < targets.length; i++) {
            const state = targets[i].getCustomState(Racero3VideoSensingBlocks.STATE_KEY);
            if (state) {
     ...
     */
    _loop () {
        const loopTime = Math.max(this.runtime.currentStepTime, Racero3VideoSensingBlocks.INTERVAL);
        ...
        if (offset > Racero3VideoSensingBlocks.INTERVAL) {
            const frame = this.runtime.ioDevices.video.getFrame({
                format: Video.FORMAT_IMAGE_DATA,
                dimensions: Racero3VideoSensingBlocks.DIMENSIONS
            });
            if (frame) {
                this._lastUpdate = time;
    ...
    /**
     * Create data for a menu in racero-blocks format, consisting of an array
    ...
    _getMotionState (target) {
        let motionState = target.getCustomState(Racero3VideoSensingBlocks.STATE_KEY);
        if (!motionState) {
            motionState = Clone.simple(Racero3VideoSensingBlocks.DEFAULT_MOTION_STATE);
            target.setCustomState(Racero3VideoSensingBlocks.STATE_KEY, motionState);
        }
        return motionState;
    }
            ...
            blocks: [
                {
                    ...
                    // threads like Racero 2's behaviour.
...
module.exports = Racero3VideoSensingBlocks;
