...
/**
 * Class for the music-related blocks in Racero 3.0
 * @param {Runtime} runtime - the runtime instantiating this block package.
 * @constructor
 */
class Racero3MusicBlocks {
    constructor (runtime) {
        /**
    ...
    /**
     * Create data for a menu in racero-blocks format, consisting of an array of objects with text and
    ...
    /**
     * An array that is a mapping from MIDI instrument numbers to Racero instrument numbers.
     * @type {number[]}
     */
    ...
    /**
     * An array that is a mapping from MIDI drum numbers in range (35..81) to Racero drum numbers.
    ...
    _getMusicState (target) {
        let musicState = target.getCustomState(Racero3MusicBlocks.STATE_KEY);
        if (!musicState) {
            musicState = Clone.simple(Racero3MusicBlocks.DEFAULT_MUSIC_STATE);
            target.setCustomState(Racero3MusicBlocks.STATE_KEY, musicState);
        }
        return musicState;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const musicState = sourceTarget.getCustomState(Racero3MusicBlocks.STATE_KEY);
            if (musicState) {
                newTarget.setCustomState(Racero3MusicBlocks.STATE_KEY, Clone.simple(musicState));
            }
        }
    }
     ...
     * This block is implemented for compatibility with old Racero projects that use the
    ...
    midiPlayDrumForBeats (args, util) {
        ...
        if (midiDescription) {
            ...
            drumNum = 2; // Default instrument used in Racero 2.0
        }
    ...
    _playDrumNum (util, drumNum) {
        ...
        if (this._concurrencyCounter > Racero3MusicBlocks.CONCURRENCY_LIMIT) {
            return;
        }
    ...
    playNoteForBeats (args, util) {
        if (this._stackTimerNeedsInit(util)) {
            ...
            note = MathUtil.clamp(note,
                Racero3MusicBlocks.MIDI_NOTE_RANGE.min, Racero3MusicBlocks.MIDI_NOTE_RANGE.max);
            let beats = Cast.toNumber(args.BEATS);
            beats = this._clampBeats(beats);
            // If the duration is 0, do not play the note. In Racero 2.0, "play drum for 0 beats" plays the drum,
     ...
     */
    _clampBeats (beats) {
        return MathUtil.clamp(beats, Racero3MusicBlocks.BEAT_RANGE.min, Racero3MusicBlocks.BEAT_RANGE.max);
    }
    ...
    /**
     * Select an instrument for playing notes according to a mapping of MIDI codes to Racero instrument numbers.
     * This block is implemented for compatibility with old Racero projects that use the 'midiInstrument:' block.
     * @param {object} args - the block arguments.
     * @param {object} util - utility object provided by the runtime.
     ...
     * the MIDI to Racero instrument mapping.
     ...
     */
    _updateTempo (tempo) {
        tempo = MathUtil.clamp(tempo, Racero3MusicBlocks.TEMPO_RANGE.min, Racero3MusicBlocks.TEMPO_RANGE.max);
...
module.exports = Racero3MusicBlocks;
