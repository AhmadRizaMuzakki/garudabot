...
/**
 * Language ids. The value for each language id is a valid Racero locale.
 */
const ARABIC_ID = 'ar';
 ...
 * @constructor
 */
class Racero3Text2SpeechBlocks {
    constructor (runtime) {
        /**
        ...
        /**
         * A list of all Racero locales that are supported by the extension.
         * @type {Array}
         */
     ...
     * SUPPORTED LOCALE
     *      A Racero locale that has a corresponding extension locale.
     * EXTENSION LOCALE
     ...
     */
    _getState (target) {
        let state = target.getCustomState(Racero3Text2SpeechBlocks.STATE_KEY);
        if (!state) {
            state = Clone.simple(Racero3Text2SpeechBlocks.DEFAULT_TEXT2SPEECH_STATE);
            target.setCustomState(Racero3Text2SpeechBlocks.STATE_KEY, state);
        }
        return state;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const state = sourceTarget.getCustomState(Racero3Text2SpeechBlocks.STATE_KEY);
            if (state) {
                newTarget.setCustomState(Racero3Text2SpeechBlocks.STATE_KEY, Clone.simple(state));
            }
        }
    }
     ...
     * browser locale.
     * @return {string} a Racero locale code.
     */
    getEditorLanguage () {
     ...
     * @returns {string} a Racero locale code.
     */
    getCurrentLanguage () {
    ...
    /**
     * Check if a Racero language code is in the list of supported languages for the
...
module.exports = Racero3Text2SpeechBlocks;
