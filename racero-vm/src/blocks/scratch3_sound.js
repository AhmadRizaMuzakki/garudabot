...
class Racero3SoundBlocks {
    ...
    _getSoundState (target) {
        let soundState = target.getCustomState(Racero3SoundBlocks.STATE_KEY);
        if (!soundState) {
            soundState = Clone.simple(Racero3SoundBlocks.DEFAULT_SOUND_STATE);
            target.setCustomState(Racero3SoundBlocks.STATE_KEY, soundState);
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const soundState = sourceTarget.getCustomState(Racero3SoundBlocks.STATE_KEY);
            if (soundState && newTarget) {
                newTarget.setCustomState(Racero3SoundBlocks.STATE_KEY, Clone.simple(soundState));
    ...
    _updateEffect (args, util, change) {
        ...
        const {min, max} = Racero3SoundBlocks.EFFECT_RANGE[effect];
...
module.exports = Racero3SoundBlocks;
