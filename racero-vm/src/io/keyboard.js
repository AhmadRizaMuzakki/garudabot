...
class Keyboard {
    ...
    /**
     * Convert from a keyboard event key name to a Racero key name.
     * @param  {string} keyString the input key string.
     * @return {string} the corresponding Racero key, or an empty string.
     */
    _keyStringToRaceroKey (keyString) {
        keyString = Cast.toString(keyString);
        // Convert space and arrow keys to their Racero key names.
        switch (keyString) {
    ...
    /**
     * Convert from a block argument to a Racero key name.
     * @param  {string} keyArg the input arg.
     * @return {string} the corresponding Racero key.
     */
    _keyArgToRaceroKey (keyArg) {
        // If a number was dropped in, try to convert from ASCII to Racero key.
    ...
    postData (data) {
        if (!data.key) return;
        const scratchKey = this._keyStringToRaceroKey(data.key);
    ...
    getKeyIsDown (keyArg) {
        ...
        const scratchKey = this._keyArgToRaceroKey(keyArg);
