 ...
 * JSON and then generates all needed racero-vm runtime structures.
 */
 ...
 * @property {Array.<Target>} targets - the imported Racero 3.0 target objects.
...
const serializeBlocks = function (blocks) {
    ...
    // a bug: LLK/racero-vm#1011, and this pass should be removed once that is
    // completely fixed)
    for (const blockID in obj) {
        const serializedBlock = obj[blockID];
...
/**
 * Parse the assets of a single "Racero object" and load them. This
 ...
 * @param {!object} object From-JSON "Racero object:" sprite, stage, watcher.
 ...
 */
const parseRaceroAssets = function (object, runtime, zip) {
...
/**
 * Parse a single "Racero object" and create all its in-memory VM objects.
 * @param {!object} object From-JSON "Racero object:" sprite, stage, watcher.
 * @param {!Runtime} runtime Runtime object to load all structures into.
 ...
 */
const parseRaceroObject = function (object, runtime, extensions, zip, assets) {
    if (!Object.prototype.hasOwnProperty.call(object, 'name')) {
        // Watcher/monitor - skip this object until those are implemented in VM.
        // @todo
        return Promise.resolve(null);
    ...
    // @todo: For now, load all Racero objects (stage/sprites) as a Sprite.
...
const deserializeMonitor = function (monitorData, runtime, targets, extensions) {
    ...
    // Due to a bug (see https://github.com/scratchfoundation/racero-vm/pull/2322), renamed list monitors may have been serialized
...
const deserialize = function (json, runtime, zip, isSingleSprite) {
    ...
    return Promise.resolve(
        targetObjects.map(target =>
            parseRaceroAssets(target, runtime, zip))
    )
        ...
        .then(assets => Promise.all(targetObjects
            .map((target, index) =>
                parseRaceroObject(target, runtime, extensions, zip, assets[index]))))
