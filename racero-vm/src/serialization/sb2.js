...
// larger block size in racero-blocks.
// @todo: Determine more precisely the right formulas here.
const WORKSPACE_X_SCALE = 1.5;
const WORKSPACE_Y_SCALE = 2.2;

/**
 * Convert a Racero 2.0 procedure string (e.g., "my_procedure %s %b %n")
 * into an argument map. This allows us to provide the expected inputs
 * to a mutated procedure call.
 * @param {string} procCode Racero 2.0 procedure string.
 ...
 * @return {Array<Array.<object>|int>} Tuple where first item is the Racero VM-format block list, and
...
/**
 * Parse a Racero object's scripts into VM blocks.
...
const parseMonitorObject = (object, runtime, targets, extensions) => {
    ...
    // video motion/direction block, this reporter is not monitorable in Racero 3.0.
    ...
    const [block, _] = parseBlock(
        [object.cmd, object.param], // Racero 2 monitor blocks only have one param.
 ...
 *   objects.
 */
const parseRaceroAssets = function (object, runtime, topLevel, zip) {
    ...
    if (object.children) {
        for (let m = 0; m < object.children.length; m++) {
            childrenAssets.push(parseRaceroAssets(object.children[m], runtime, false, zip));
        }
    }
 ...
 */
const parseRaceroObject = function (object, runtime, extensions, topLevel, zip, assets) {
    if (!Object.prototype.hasOwnProperty.call(object, 'objName')) {
    ...
    if (Object.prototype.hasOwnProperty.call(object, 'scriptComments')) {
        const comments = object.scriptComments.map(commentDesc => {
            ...
            const newComment = new Comment(
                ...
                // If it's a block comment, we'll let racero-blocks handle positioning
    ...
    if (object.children) {
        for (let m = 0; m < object.children.length; m++) {
            childrenPromises.push(
                parseRaceroObject(object.children[m], runtime, extensions, false, zip, assets.children[m])
            );
        }
    }
...
const sb2import = function (json, runtime, optForceSprite, zip) {
    ...
    return Promise.resolve(parseRaceroAssets(json, runtime, !optForceSprite, zip))
        // Force this promise to wait for the next loop in the js tick. Let
        // storage have some time to send off asset requests.
        .then(assets => Promise.resolve(assets))
        .then(assets => (
            parseRaceroObject(json, runtime, extensions, !optForceSprite, zip, assets)
        ))
 ...
 * list of all blocks for the target
 * @return {Array.<object|int>} Tuple where first item is the Racero VM-format block (or null if unsupported object),
...
const parseBlock = function (sb2block, addBroadcastMsg, getVariableId, extensions, parseState, comments, commentIndex) {
    ...
    // non-positional named Racero VM arguments.
    for (let i = 0; i < blockMetadata.argMap.length; i++) {
        ...
        if (expectedArg.type === 'input') {
            ...
            if (expectedArg.inputOp === 'math_number' ||
                ...
                fieldName = 'NUM';
                // Fields are given Racero 2.0 default values if obscured.
                if (shadowObscured) {
                    fieldValue = 10;
                }
