...
if (window.performance) {
    ...
    performance.mark('Racero.EvalStart');
}
...
const RaceroStorage = require('scratch-storage');
const VirtualMachine = require('..');
const Runtime = require('../engine/runtime');

const RaceroRender = require('scratch-render');
const AudioEngine = require('racero-audio');
const RaceroSVGRenderer = require('scratch-svg-renderer');

const Racero = window.Racero = window.Racero || {};
...
const loadProject = function () {
    ...
    Racero.vm.downloadProjectId(id);
    return id;
};
    ...
    on (storage, vm) {
        ...
        storage.webHelper.load = function (...args) {
            if (_this.dataLoaded === 0 && window.performance) {
                ...
                performance.mark('Racero.LoadDataStart');
            }
            ...
            if (_this.dataLoaded) {
                if (_this.contentTotal === 0 && window.performance) {
                    performance.mark('Racero.DownloadStart');
                }
            ...
            result.then(() => {
                if (_this.dataLoaded === 0) {
                    if (window.performance) {
                        ...
                        performance.mark('Racero.LoadDataEnd');
                        performance.measure('Racero.LoadData', 'Racero.LoadDataStart', 'Racero.LoadDataEnd');
                    }

                    _this.dataLoaded = 1;

                    window.RaceroVMLoadDataEnd = Date.now();
                } else {
                ...
                if (_this.contentComplete && _this.contentComplete === _this.contentTotal) {
                    if (window.performance) {
                        ...
                        performance.mark('Racero.DownloadEnd');
                        performance.measure('Racero.Download', 'Racero.DownloadStart', 'Racero.DownloadEnd');
                    }

                    window.RaceroVMDownloadEnd = Date.now();
                }
        ...
        vm.runtime.on(Runtime.PROJECT_LOADED, () => {
            ...
            if (window.performance) {
                ...
                // all the project assets?
                performance.mark('Racero.LoadEnd');
                performance.measure('Racero.Load', 'Racero.LoadStart', 'Racero.LoadEnd');
            }

            window.RaceroVMLoadEnd = Date.now();
...
const runBenchmark = function () {
    ...
    const storage = new RaceroStorage();
    ...
    const renderer = new RaceroRender(canvas);
    Racero.renderer = renderer;
    vm.attachRenderer(renderer);
    const audioEngine = new AudioEngine();
    vm.attachAudioEngine(audioEngine);
    vm.attachV2BitmapAdapter(new RaceroSVGRenderer.BitmapAdapter());
    ...
    document.addEventListener('mousemove', e => {
        ...
        Racero.vm.postIOData('mouse', coordinates);
    });
    canvas.addEventListener('mousedown', e => {
        ...
        Racero.vm.postIOData('mouse', data);
        e.preventDefault();
    });
...
if (window.performance) {
    performance.mark('Racero.EvalEnd');
    performance.measure('Racero.Eval', 'Racero.EvalStart', 'Racero.EvalEnd');
}

window.RaceroVMEvalEnd = Date.now();
