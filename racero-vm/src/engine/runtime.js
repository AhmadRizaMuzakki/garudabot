...
const RaceroBlocksConstants = require('./racero-blocks-constants');
...
const RaceroLinkWebSocket = require('../util/scratch-link-websocket');
...
/**
 * Information used for converting Racero argument types into racero-blocks data.
...
const ArgumentTypeMap = (() => {
    ...
    map[ArgumentType.ANGLE] = {
        shadow: {
            ...
            // the racero-blocks defaults for that field will be
...
class Runtime extends EventEmitter {
    constructor () {
         ...
         * Racero community, such as CSFirst.
         * @type {?string}
         */
        this.origin = null;

        this._initRaceroLink();
    ...
    _fillExtensionCategory (categoryInfo, extensionInfo) {
        ...
        for (const menuName in extensionInfo.menus) {
            if (Object.prototype.hasOwnProperty.call(extensionInfo.menus, menuName)) {
                ...
                const convertedMenu = this._buildMenuForRaceroBlocks(menuName, menuInfo, categoryInfo);
        ...
        for (const blockInfo of extensionInfo.blocks) {
            try {
                const convertedBlock = this._convertForRaceroBlocks(blockInfo, categoryInfo);
    ...
    /**
     * Convert the given extension menu items into the racero-blocks style of list of pairs.
    ...
    /**
     * Build the racero-blocks JSON for a menu. Note that racero-blocks treats menus as a special kind of block.
     * @param {string} menuName - the name of the menu
     ...
     * @returns {object} - a JSON-esque object ready for racero-blocks' consumption
     * @private
     */
    _buildMenuForRaceroBlocks (menuName, menuInfo, categoryInfo) {
        ...
        return {
            json: {
                ...
                outputShape: menuInfo.acceptReporters ?
                    RaceroBlocksConstants.OUTPUT_SHAPE_ROUND : RaceroBlocksConstants.OUTPUT_SHAPE_SQUARE,
                args0: [
                    {
                        type: 'field_dropdown',
                        name: menuName,
    ...
    _buildCustomFieldInfo (fieldName, fieldInfo, extensionId, categoryInfo) {
        ...
        return {
            ...
            scratchBlocksDefinition: this._buildCustomFieldTypeForRaceroBlocks(
                extendedName,
                fieldInfo.output,
    ...
    /**
     * Build the racero-blocks JSON needed for a fieldType.
     ...
     * @param {number} outputShape - Shape of the field (from RaceroBlocksConstants)
     * @param {object} categoryInfo - The category the field belongs to (Used to set its colors)
     * @returns {object} - Object to be inserted into racero-blocks
     */
    _buildCustomFieldTypeForRaceroBlocks (fieldName, output, outputShape, categoryInfo) {
        return {
            json: {
                type: fieldName,
                message0: '%1',
    ...
    /**
     * Convert ExtensionBlockMetadata into data ready for racero-blocks.
     * @param {ExtensionBlockMetadata} blockInfo - the block info to convert
     * @param {CategoryInfo} categoryInfo - the category for this block
     * @returns {ConvertedBlockInfo} - the converted & original block information
     * @private
     */
    _convertForRaceroBlocks (blockInfo, categoryInfo) {
        if (blockInfo === '---') {
        ...
        return this._convertBlockForRaceroBlocks(blockInfo, categoryInfo);
    }

    /**
     * Convert ExtensionBlockMetadata into racero-blocks JSON & XML, and generate a proxy function.
     * @param {ExtensionBlockMetadata} blockInfo - the block to convert
     * @param {CategoryInfo} categoryInfo - the category for this block
     * @returns {ConvertedBlockInfo} - the converted & original block information
     * @private
     */
    _convertBlockForRaceroBlocks (blockInfo, categoryInfo) {
        ...
        case BlockType.COMMAND:
            blockJSON.outputShape = RaceroBlocksConstants.OUTPUT_SHAPE_SQUARE;
        ...
        case BlockType.REPORTER:
            ...
            blockJSON.outputShape = RaceroBlocksConstants.OUTPUT_SHAPE_ROUND;
            break;
        case BlockType.BOOLEAN:
            blockJSON.output = 'Boolean';
            blockJSON.outputShape = RaceroBlocksConstants.OUTPUT_SHAPE_HEXAGONAL;
            break;
        case BlockType.HAT:
        ...
        let outLineNum = 0; // used for racero-blocks `message${outLineNum}` and `args${outLineNum}`
     ...
     * @private
     */
    _convertSeparatorForRaceroBlocks (blockInfo) {
        return {
            info: blockInfo,
            xml: '<sep gap="36"/>'
        };
    ...
    /**
     * Convert a button for racero-blocks. A button has no opcode but specifies a callback name in the `func` field.
     ...
     * @private
     */
    _convertButtonForRaceroBlocks (buttonInfo) {
        // for now we only support these pre-defined callbacks handled in racero-blocks
     ...
     * @return {object} JSON blob for a racero-blocks image field.
     * @private
     */
    ...
    /**
     * Helper for _convertForRaceroBlocks which handles linearization of argument placeholders. Called as a callback
     * from string#replace. In addition to the return value the JSON and XML items in the context will be filled.
     * @param {object} context - information shared with _convertForRaceroBlocks about the block, etc.
     ...
     * @return {string} racero-blocks placeholder for the argument: '%1'.
     * @private
     */
    _convertPlaceholders (context, match, placeholder) {
        ...
        // Start to construct the racero-blocks style JSON defining how the block should be
        // laid out
        let argJSON;
        ...
        if (argTypeInfo.fieldType === 'field_image') {
            ...
            // <value> is the RaceroBlocks name for a block input.
            if (valueName) {
    ...
    /**
     * @returns {Array.<object>} racero-blocks XML for each category of extension blocks, in category order.
    ...
    /**
     * @returns {Array.<string>} - an array containing the racero-blocks JSON information for each dynamic block.
     */
    getBlocksJSON () {
    ...
    /**
     * One-time initialization for Racero Link support.
     */
    _initRaceroLink () {
        ...
        if (typeof self !== 'undefined' &&
            ...
            // Create a script tag for the Racero Link browser extension, unless one already exists
     ...
     * @returns {RaceroLinkSocket} The scratch link socket.
     */
    getRaceroLinkSocket (type) {
        const factory = this._linkSocketFactory || this._defaultRaceroLinkSocketFactory;
        return factory(type);
    }

    /**
     * Configure how RaceroLink sockets are created. Factory must consume a "type" parameter
     * either BT or BLE.
     * @param {Function} factory The new factory for creating RaceroLink sockets.
     */
    configureRaceroLinkSocketFactory (factory) {
     ...
     * @param {string} type Either BLE or BT
     * @returns {RaceroLinkSocket} The new scratch link socket (a WebSocket object)
     */
    _defaultRaceroLinkSocketFactory (type) {
        const Racero = self.Racero;
        const RaceroLinkSafariSocket = Racero && Racero.RaceroLinkSafariSocket;
        // detect this every time in case the user turns on the extension after loading the page
        const useSafariSocket = RaceroLinkSafariSocket && RaceroLinkSafariSocket.isSafariHelperCompatible();
        return useSafariSocket ? new RaceroLinkSafariSocket(type) : new RaceroLinkWebSocket(type);
    }
     ...
     * bitmaps to scratch 3 bitmaps. (Racero 3 bitmaps are all bitmap resolution 2)
     ...
     * @param {!RaceroStorage} storage The storage module to attach
     */
    ...
    startHats (requestedHatOpcode,
        ...
        // For compatibility with Racero 2, edge triggered hats need to be processed before
        // threads are stepped. See RaceroRuntime.as for original implementation
    ...
    dispose () {
        ...
        if (this.extensions) {
            const aiExtensions = ['fingers', 'expresive', 'recognition'];
            aiExtensions.forEach(id => {
                const extensionInstance = this.extensions.getExtensionInstance(id);
                if (extensionInstance && typeof extensionInstance.reset === 'function') {
                    try {
                        extensionInstance.reset();
                    } catch (err) {
                        console.error(`Failed to reset AI extension ${id}:`, err);
                    }
                }
            });
        }
    ...
    /**
     * Get a target representing the Racero stage, if one exists.
     * @return {?Target} The target, if found.
     */
    getTargetForStage () {
        for (let i = 0; i < this.targets.length; i++) {
     ...
     * This value is helpful in certain instances for compatibility with Racero 2,
